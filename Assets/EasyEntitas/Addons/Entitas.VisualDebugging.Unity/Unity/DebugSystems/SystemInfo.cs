        ...
        public SystemInfo(ISystem system) {
            ...
            if (debugSystem != null) {
				...
				if (systemType.IsGenericType) {
					_systemName = systemType.UnderlyingSystemType.ToString();
				}
				_systemName = _systemName.EndsWith(SYSTEM_SUFFIX, StringComparison.Ordinal)
                    ? _systemName.Substring(0, systemType.Name.Length - SYSTEM_SUFFIX.Length)
                    : _systemName;
            }
