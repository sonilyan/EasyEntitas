...
public class Feature : Entitas.Systems {
    ...
    System.Collections.Generic.Dictionary<IExecuteSystem,string> _executeSystemNames;
	...
	public Feature(Systems systems) : base(systems) {
		_initializeSystemNames = new System.Collections.Generic.List<string>();
		_executeSystemNames = new System.Collections.Generic.Dictionary<IExecuteSystem, string>();
    ...
    public override Entitas.Systems Add(Entitas.ISystem system) {
        ...
        if (system is Entitas.IExecuteSystem) {
            _executeSystemNames.Add(system as IExecuteSystem, systemName);
        }
    ...
    public override void Execute() {
        for (int i = 0; i < _executeSystems.Count; i++) {
			...
			if (!_executeSystemNames.ContainsKey(system))
				_executeSystemNames.Add(system,system.GetType().Name);

			UnityEngine.Profiling.Profiler.BeginSample(_executeSystemNames[system]);
			system.Execute();
            UnityEngine.Profiling.Profiler.EndSample();
        }
    }
...
public class Feature : Entitas.VisualDebugging.Unity.DebugSystems {
		...
		public Feature(Systems systems) : base(systems) {
			initialize(systems.GetType().Name);
		}
	}
#else
