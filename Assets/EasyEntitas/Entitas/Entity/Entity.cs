...
namespace Entitas {
    ...
    /// You can add, replace and remove IComponentBase to an entity.
    public partial class Entity : IEntity {
		...
		public event EntityComponentBeforeUpdated OnComponentBeforeUpdated;
        ...
        public int contextIndex { get { return context.ContextIndex; } }
        ...
        IComponentBase[] _components;
        Stack<IComponentBase>[] _componentPools;
        ContextInfo _contextInfo;
        IAERC _aerc;

        IComponentBase[] _componentsCache;
        ...
        public void Initialize(int creationIndex, int totalComponents, Stack<IComponentBase>[] componentPools, ContextInfo contextInfo = null, IAERC aerc = null) {
			Reactivate(creationIndex);

            _totalComponents = totalComponents * 3;
            _components = new IComponentBase[totalComponents * 3];
		...
		public void AddComponent(int index, IComponentBase component) {
            if (!_isEnabled) {
        ...
        /// The prefered way is to use the
        /// generated methods from the code generator.
        public void ReplaceComponent(int index, IComponentBase component,Action invokeReplace) {
            if (!_isEnabled) {
                throw new EntityIsNotEnabledException(
            ...
            if (HasComponent(index)) {
	            beforeReplaceComponent(index, component);
				invokeReplace?.Invoke();
	    ...
	    void beforeReplaceComponent(int index, IComponentBase replacement) {
            var previousComponent = _components[index];
		    if (replacement == previousComponent) {
			    if (OnComponentBeforeUpdated != null) {
				    OnComponentBeforeUpdated(this, index, replacement);
			    }
		    }
	    }

	    void replaceComponent(int index, IComponentBase replacement) {
            _toStringCache = null;
            var previousComponent = _components[index];
            if (replacement != previousComponent) {
        ...
        /// The prefered way is to use the
        /// generated methods from the code generator.
        public IComponentBase GetComponent(int index) {
            if (!HasComponent(index)) {
                throw new EntityDoesNotHaveComponentException(
        ...
        public IComponentBase[] GetComponents() {
        ...
        /// Use entity.CreateComponent(index, type) to get a new or
        /// reusable component from the componentPool.
        public Stack<IComponentBase> GetComponentPool(int index) {
            var componentPool = _componentPools[index];
            if (componentPool == null) {
                componentPool = new Stack<IComponentBase>();
        ...
        public IComponentBase CreateComponent(int index, Type type) {
            var componentPool = GetComponentPool(index);
            return componentPool.Count > 0
                        ? componentPool.Pop()
                        : (IComponentBase)Activator.CreateInstance(type);
        }
            ...
            if (_toStringCache == null) {
                ...
                for (int i = 0; i < components.Length; i++) {
                    ...
                    var implementsToString = type.GetMethod("ToString")
                                                 .DeclaringType.ImplementsInterface<IComponentBase>();
