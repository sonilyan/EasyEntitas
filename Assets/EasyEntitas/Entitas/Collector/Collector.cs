...
namespace Entitas {
    ...
    public class Collector : ICollector {
        ...
        GroupUpdated _updateEntityCache;
            ...
            for (int i = 0; i < _groups.Length; i++) {
                ...
                switch (groupEvent) {
                    case GroupEvent.Added:
                        ...
                        group.OnEntityAdded += _addEntityCache;
						break;
					case GroupEvent.AddedOrUpdated:
						group.OnEntityAdded -= _addEntityCache;
						group.OnEntityAdded += _addEntityCache;
						group.OnEntityUpdated -= _updateEntityCache;
						group.OnEntityUpdated += _updateEntityCache;
						break;
                    case GroupEvent.Updated:
	                    group.OnEntityUpdated -= _updateEntityCache;
	                    group.OnEntityUpdated += _updateEntityCache;
						break;
		...
		void updateEntity(IGroup group, Entity entity, int index, IComponentBase previousComponent, IComponentBase newComponent) {
			addEntity(group, entity, index, newComponent);
		}

		void addEntity(IGroup group, Entity entity, int index, IComponentBase component) {
