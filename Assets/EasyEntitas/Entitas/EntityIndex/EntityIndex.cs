...
namespace Entitas {
    ...
    public class EntityIndex<TKey> : AbstractEntityIndex<TKey> {
        ...
        public EntityIndex(string name, IGroup group, Func<Entity, IComponentBase, TKey> getKey) : base(name, group, getKey) {
            _index = new Dictionary<TKey, HashSet<Entity>>();
            Activate();
        }

        public EntityIndex(string name, IGroup group, Func<Entity, IComponentBase, TKey[]> getKeys) : base(name, group, getKeys) {
            _index = new Dictionary<TKey, HashSet<Entity>>();
            Activate();
        }

        public EntityIndex(string name, IGroup group, Func<Entity, IComponentBase, TKey> getKey, IEqualityComparer<TKey> comparer) : base(name, group, getKey) {
            _index = new Dictionary<TKey, HashSet<Entity>>(comparer);
            Activate();
        }

        public EntityIndex(string name, IGroup group, Func<Entity, IComponentBase, TKey[]> getKeys, IEqualityComparer<TKey> comparer) : base(name, group, getKeys) {
            _index = new Dictionary<TKey, HashSet<Entity>>(comparer);
            Activate();
        }
