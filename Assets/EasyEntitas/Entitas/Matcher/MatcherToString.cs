    ...
    public partial class Matcher {
        ...
        static void appendIndices(StringBuilder sb, string prefix, int[] indexArray, string[] componentNames) {
	        ...
	        for (int i = 0; i < indexArray.Length; i++) {
		        var index = indexArray[i] % componentNames.Length;
		        var typeIndex = indexArray[i] / componentNames.Length;
		        if (componentNames == null) {
			        sb.Append(index);
		        }
		        else if (typeIndex != 0) {
			        var typename = typeIndex == 1 ? "Add" : "Remove";
			        sb.Append($"{typename}<{componentNames[index]}>");
		        }
		        else {
