...
namespace Entitas {
    ...
    public partial class Matcher {
        ...
        internal static IAllOfMatcher AllOf(params int[] indices) {
            var matcher = new Matcher();
            matcher._allOfIndices = distinctIndices(indices);
            return matcher;
        }

		internal static IAllOfMatcher AllOf(params IMatcher[] matchers) {
            var allOfMatcher = (Matcher)Matcher.AllOf(mergeIndices(matchers));
            setComponentNames(allOfMatcher, matchers);
            return allOfMatcher;
        }

		internal static IAnyOfMatcher AnyOf(params int[] indices) {
            var matcher = new Matcher();
            matcher._anyOfIndices = distinctIndices(indices);
            return matcher;
        }

		internal static IAnyOfMatcher AnyOf(params IMatcher[] matchers) {
